...
import { findCareerMatchesForChanger, saveSurveyDataForChanger, completeChangerSurvey } from '../../components/careerChangerMatching';

const HorizonChangerSurvey = () => {
  // User selections
  ...
  const handleSurveySubmit = async () => {
    ...
    try {
      ...
      await saveSurveyDataForChanger({
      ...
      // Find career matches based on selections, excluding current path
      const matches = await findCareerMatchesForChanger({
        currentPath: currentCareerPath,
        skills: selectedSkills,
        interests: selectedInterests,
        degrees,
  ...
  const handleSelectPath = async (pathId: string) => {
    try {
      setLoading(true);
      const success = await completeChangerSurvey(pathId);
    ...
    <View className="flex-1 bg-white">
      <View className="flex-row items-center px-4 pt-4 pb-2">
        ...
        <View className="flex-1 items-center">
          <Text className="text-lg font-bold text-gray-800">Change Career Path</Text>
        </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View className="mb-4">
              <Text className="text-xl font-bold text-gray-800">Your New Career Matches</Text>
              <Text className="text-gray-600 mt-1">
                Based on your experience and interests, here are some new career paths that might be a good fit for you.
              </Text>
              {currentCareerPath && (
                <View className="mt-4 p-3 bg-blue-50 rounded-md">
                  <Text className="text-blue-800">
                    Your current career path: <Text className="font-bold">{currentCareerPath}</Text>
                  </Text>
                </View>
              )}
            </View>
        ...
        <ScrollView className="flex-1 px-4">
          <Text className="text-base text-gray-600 mb-4">
            Looking for a career change? Tell us about your current role, skills, and interests to find new paths.
          </Text>
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            <Text className="text-white font-semibold text-lg">Find New Career Paths</Text>
          </TouchableOpacity>
        </ScrollView>
      )}
...
export default HorizonChangerSurvey;